...
- `verbs.html` — verb recall + conjugation drill (TTMIK Level 1 Lessons 17–18, Level 2 Lesson 1). Cards are verbs with a `conjugations` map keyed by tense (`present`, `past`, `future`, `want`), each value `{korean, romanization}` or `null`. Three modes selected by a toggle: **Recall** (prompt → base + full conjugation table), **Conjugate** (base + tense chip → type the conjugated form) and **Full drill** (one card walks the `CHAIN` — dictionary form, then future, past, present — each typed and graded in turn, with a step list on the front and a marked-up table on the back). A second pill, `direction` (`en-ko` / `ko-en`), swaps the prompt language in Recall and Full drill; in Full drill it also turns the first step into "type the English meaning", graded by `isCorrectEnglish()` (any comma-separated gloss, optional "to"/"be", bracketed notes ignored). The pill is hidden in Conjugate mode, which always asks for Korean. The tense filter (`tense-filters`) is only visible in Conjugate mode via `body.mode-conjugate` and controls which form is drilled
...
- What each deck speaks: `index.html`/`numbers.html` the card's hangul; `verbs.html` the base form plus a button per conjugation row (Conjugate mode speaks the drilled form; Full drill withholds the base form until it is on screen); `particles.html` the sentence with its blanks filled in
